...
Код нижче генерує випадкові тривимірні точки у декартових координатах (x, y, z) у діапазоні від -10 до 10. Потім він перетворює ці точки у сферичні координати (радіус, азимут, полярний кут) і обчислює відстані між кожною парою сусідніх точок. Відстані розраховуються за трьома різними методами: у декартових координатах, у полярних координатах (двовимірний випадок), і в сферичних координатах як по об'єму сфери, так і по поверхні сфери.
...
На скріншотах видно, скільки часу знадобилося на обчислення відстаней між 10 000 пікселів у різних системах координат. У Декартовій системі координат обчислення зайняли близько 11 000 мс, а Полярній — близько 12 000 мс. Це показує, що розрахунки у Полярній системі вимагають більше часу через перетворення координат. У сферичній системі координат, коли обчислення виконуються за обсягом, час був приблизно таким самим, як і в Декартовій системі, близько 12 000 мс. Однак обчислення по поверхні сфери зайняли найменше часу, близько 8 000 мс. Це показує, що розрахунок відстаней по поверхні сфери ефективніший з погляду продуктивності.
